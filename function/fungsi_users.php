...
try {
    if ($aksi == 'simpan') {
        ...
        if (password_verify($passwordlama, $cek['password'])) {
            ...
            mysqli_query($conn, "UPDATE users SET password = '$passwordbaru' WHERE user_id = '$user_id'");
            $_SESSION['flash'] = [
                'icon' => 'success',
    ...
    // ✅ Redirect jika sukses (global handler)
    if (!empty($_SERVER['HTTP_REFERER'])) {
        header("Location: " . $_SERVER['HTTP_REFERER']);
    } else {
